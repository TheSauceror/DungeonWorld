...
function init() {
  ...
  $party = mysqli_query($conn,"SELECT * FROM Hero WHERE heroparty = '$hero[heroparty]'");
...
echo "<br>";
  ...
  foreach(explode("|", $roomstats[$key]['enemies']) as $enemyname) {
    $fighters[] = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM Enemies WHERE enemyname = '$enemyname'"));
  }
  mysqli_close($conn);
  //findMaxHpAndMp();
  ...
  switch($testplan) {
    ...
    case "magicattack":
      ...
      if((abs($fighters[$i]['x']-$fighters[$closest]['x']) + abs($fighters[$i]['y']-$fighters[$closest]['y'])) < 6) {
        $report .= $fighters[$i]['name'] . " zaps " . $fighters[$closest]['name'] . " for " . $fighters[$i]['int'] . " damage.<br>";
        $fighters[$closest]['hp'] -= $fighters[$i]['int'];
        if($fighters[$closest]['hp'] < 1) {
          $fighters[$closest]['hp'] = 0;
          $map[$fighters[$closest]['y']][$fighters[$closest]['x']] = -1;
          $report .= $fighters[$closest]['name'] . " dies.<br>";
...
function findMaxHpAndMp() {
  ...
  foreach($partyfighters as &$partyfighters) {
    ...
    $partyfighters['maxhp'] = floor(($partyfighters['con']*5 + $partyfighters['str']*3) * $hpmult);
    $partyfighters['hp'] = $partyfighters['maxhp'];
    $partyfighters['maxmp'] = floor(($partyfighters['int']*5 + $partyfighters['wis']*3) * $mpmult);
  ...
  for ($i=0; $i < $w; $i++) {
    for ($j=0; $j < $l; $j++) {
      ...
      if($key > -1 && $fighters[$key]['hp'] > 0) {
        $mapstring .= "<img src='images/" . $fighters[$key]['heroprof'] . ".gif' height='24' width='24' title='" . $fighters[$key]['heroprof'] . "'>";
      }
      else {
