...
$items = mysqli_query($conn,"SELECT * FROM Inventory LEFT JOIN ItemBase ON Inventory.base = ItemBase.baseid Left JOIN ItemPrefix ON Inventory.prefix = ItemPrefix.prefixid LEFT JOIN ItemSuffix ON Inventory.suffix = ItemSuffix.suffixid WHERE Inventory.owner = $cookie[0] AND Inventory.market = '0'");
echo "<table><tr><th>ID</th><th>Prefix</th><th>Base</th><th>Suffix</th><th>Slot</th><th>S. Dmg</th><th>P. Dmg</th><th>B. Dmg</th><th>S. Arm</th><th>P. Arm</th><th>B. Arm</th><th>Description</th><th>Sell Item</th></tr>";

while($row = mysqli_fetch_assoc($items)) {
  echo "<tr><td>" . $row['inventoryid'] . "</td><td>" . $row['prefixname'] . "</td><td>" . $row['basename'] . "</td><td>" . $row['suffixname'] . "</td><td>" . $row['slot'] . "</td><td>" . getItemStats($row['inventoryid'], "sdam") . "</td><td>" . getItemStats($row['inventoryid'], "pdam") . "</td><td>" . getItemStats($row['inventoryid'], "bdam") . "</td><td>" . getItemStats($row['inventoryid'], "sarm") . "</td><td>" . getItemStats($row['inventoryid'], "parm") . "</td><td>" . getItemStats($row['inventoryid'], "barm") . "</td><td>" . $row['basedes'] . " " . $row['prefixdes'] . " " . $row['suffixdes'] . "</td><td>" . getItemPrice($row['inventoryid']) . " gold</td></tr>";
}
echo "</table>";
...
function getItemPrice($itemid) {
  ...
  mysqli_close($conn);
  return max(1, floor((($item['prefixvalue'] * $item['prefixlevel']) + ($item['basevalue'] * $item['baselevel']) + ($item['suffixvalue'] * $item['suffixlevel'])) / 2));
}
