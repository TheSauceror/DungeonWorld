<head><title>Adventures Of Eld - Profile</title></head>
...
if(isset($_POST['mainhand'])) {
  ...
  if(is_null(mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM Inventory LEFT JOIN ItemBase ON Inventory.base = ItemBase.baseid Left JOIN ItemPrefix ON Inventory.prefix = ItemPrefix.prefixid LEFT JOIN ItemSuffix ON Inventory.suffix = ItemSuffix.suffixid WHERE Inventory.inventoryid = '$_POST[mainhand]' AND ItemBase.slot = 'Two Hand'")))) {
...
if(isset($_POST['attribute'])) {
  ...
  $cost = $hero[$aID] * 100;
  if($hero['gold'] < $cost) { echo "<div class='alert'>Not enough gold!</div>"; } else {
...
if($id == $cookie[0]) {
  ...
  echo "<table>";
  echo "<tr><td>Main Hand:</td><td><select name='mainhand' onChange='this.form.submit();'>"; getAllSlot($id, $hero['dungeonlevel'], 'Two Hand', 1); echo "</select></td><td>" . getItemDes(0, $id, 'Two Hand', 1) . "</td></tr>";
...
function getAllSlot($hero, $level, $slot, $equip) {
  ...
  if($slot == 'Two Hand') {
    ...
    $query = "SELECT * FROM Inventory LEFT JOIN ItemBase ON Inventory.base = ItemBase.baseid Left JOIN ItemPrefix ON Inventory.prefix = ItemPrefix.prefixid LEFT JOIN ItemSuffix ON Inventory.suffix = ItemSuffix.suffixid WHERE Inventory.owner = '$hero' AND Inventory.time = '0' AND Inventory.baselevel <= '$level' AND ItemBase.slot = '$slot'";
  }
  //$query = "SELECT * FROM Inventory LEFT JOIN ItemBase ON Inventory.base = ItemBase.baseid Left JOIN ItemPrefix ON Inventory.prefix = ItemPrefix.prefixid LEFT JOIN ItemSuffix ON Inventory.suffix = ItemSuffix.suffixid WHERE Inventory.owner = '$hero' AND Inventory.time = '0' AND Inventory.baselevel <= '$level' AND (ItemBase.slot = 'Off Hand' OR ItemBase.slot = 'Hand')";
  $slotitems = mysqli_query($conn, $query);
